...
export class ProgressGateway {
  ...
  emitSceneContentUpdated(
    storyId: string,
    payload: { sceneId: string; versionId: string; wordCount: number },
  ): void {
    this.logger.debug(
      `Emitting scene:content-updated for story=${storyId} scene=${payload.sceneId}`,
    );
    this.server.to(storyId).emit('scene:content-updated', {
      storyId,
      ...payload,
    });
  }
